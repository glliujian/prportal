		...
		$(document).ready(function() {
			validateForm = $("#inputForm").validate({
				submitHandler: function(form){
					...
					//防止用戶多點
					$('#btnBackToPrevious').attr('disabled','disabled');
					$('#btnBackToInitiator').attr('disabled','disabled');
	...
	<form:form id="inputForm" modelAttribute="pr" action="${ctx}/oa/pr/saveAudit" method="post" class="form-horizontal">
		...
		<table id="tableContent" class="table table-bordered  table-condensed dataTables-example dataTable no-footer">
		   <tbody>
				...
				<%--<tr>
					...
					<td colspan=3 class="width-35">
						<form:input path="paidByFinance" value="${fns:getDictLabel(pr.budgetType,'IT_PR_PAID','')}"  readonly="true"  cssClass="form-control input-sm " />
				...
				<tr>
					<td colspan=6 class="width-85">
						<form:textarea path="purchasePurpose" class="form-control required" readonly="true" rows="3" maxlength="1000" htmlEscape="false" />
					</td>
				</tr>
				<!-- 
				<tr>
		...
		<div class="form-actions" style="padding:0;text-align:center;">
				<c:if test="${pr.act.taskDefKey ne 'apply_end'}">
					...
					<input id="btnBackToPrevious" class="btn btn-inverse" type="submit" value="<spring:message code='驳回到上一级'></spring:message>" onclick="$('#flag').val(2)"/>&nbsp; 
					<input id="btnBackToInitiator" class="btn btn-inverse" type="submit" value="<spring:message code='驳回到申请者'></spring:message>" onclick="$('#flag').val(0)"/>&nbsp;
				</c:if>
