		...
		$(document).ready(function() {
			...
			validateForm = $("#inputForm").validate({
				submitHandler: function(form){
					...
					var index = loading("<spring:message code='正在提交，请稍等...'></spring:message>");
					form.submit();
					//防止用戶多點
					$('#btnSave').attr('disabled','disabled');
					$('#btnSubmit').attr('disabled','disabled');
...
<body class="gray-bg">
	...
	<form:form id="inputForm" modelAttribute="pr" action="${ctx}/oa/pr/saveModify" method="post" class="form-horizontal" enctype="multipart/form-data">
		<form:hidden path="id"/>
		...
		<table class="table table-bordered  table-condensed dataTables-example dataTable no-footer">
		   <tbody>
		   		...
		   		<tr>
					...
					<td class="width-15 active"><label class="pull-right"><spring:message code="日期"></spring:message>:</label></td>
					<td class="width-35">
						<input type="text" maxlength="20" class="laydate-icon form-control layer-date " readonly="true"
							value="<fmt:formatDate value="${pr.createDate}" pattern="yyyy-MM-dd"/>"/>
					</td>
		   		</tr>
				<tr>
				    <td class="width-15 active"><label class="pull-right"><font color="red">*</font><spring:message code="部门名称"></spring:message>:</label></td>
					<td colspan=3 class="width-35">
						<form:input path="applyUserDepartmentName" id="applyUserDepartmentName" htmlEscape="false"  readonly="true"   class="form-control required"/>
					</td> 
					<!-- <td class="width-15 active"><label class="pull-right"><spring:message code="成本中心编号"></spring:message>:</label></td>
					<td class="width-35">
						
						<form:input path="costCenter" htmlEscape="false" maxlength="50"  class="form-control"/> 
					</td> -->
					<td class="width-15 active"><label class="pull-right"><spring:message code="成本中心编号"></spring:message>:</label></td>
					<td >
				...
				<tr>
					...
					<td class="width-15">
						<%--<form:input path="unitPrice" id="unitPrice" htmlEscape="false"  onblur="caculateTotal()"  class="form-control required round"/> --%>
					
						<input name="unitPrice" id="unitPrice" type="text" class="form-control required number round" onblur="caculateTotal()" value="<fmt:formatNumber value="${pr.unitPrice}" pattern="0.00"/>"/>
					</td>
					...
					<td class="width-35">
						<form:input path="quantity" id="quantity" htmlEscape="false"  onblur="caculateTotal()"  class="form-control required"/>
					</td>
				</tr>
				<tr>
					...
					<td class="width-35">
						<form:input path="equipmentDesciption" id="equipmentDesciption" htmlEscape="false" maxlength="50"   class="form-control "/>
					</td>
				</tr>
				<!-- 
				<tr>
				...
				<tr>
					<td colspan=6 class="width-85">
						<form:textarea path="paymentSpecial" class="form-control "  rows="3" maxlength="1000" htmlEscape="false" />
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<c:if test="${not empty pr.attachment}">
							...
							<a href="${ctx}/oa/pr/deleteAttachment?attachment=${fns:urlEncode(pr.attachment)}&applicationNo=${pr.applicationNo}" onclick="return confirmx('<spring:message code="确认要删除该附件吗？"></spring:message>', this.href)" ><i class="fa fa-trash"></i> </a>
					</c:if>
					<c:if test="${empty pr.attachment}">
				...
				<tr>
					...
					<c:if test="${not empty pr.attachment2}">
							...
							<a href="${ctx}/oa/pr/deleteAttachment?attachment2=${fns:urlEncode(pr.attachment2)}&applicationNo=${pr.applicationNo}" onclick="return confirmx('<spring:message code="确认要删除该附件吗？"></spring:message>', this.href)" ><i class="fa fa-trash"></i> </a>
					</c:if>
					<c:if test="${empty pr.attachment2}">
				...
				<tr>
					...
					<c:if test="${not empty pr.attachment3}">
							...
							<a href="${ctx}/oa/pr/deleteAttachment?attachment3=${fns:urlEncode(pr.attachment3)}&applicationNo=${pr.applicationNo}" onclick="return confirmx('<spring:message code="确认要删除该附件吗？"></spring:message>', this.href)" ><i class="fa fa-trash"></i> </a>
					</c:if>
					<c:if test="${empty pr.attachment3}">
				...
				<tr>
					...
					<td colspan=5 class="width-15 active">
						<c:forEach items="${pr.attachmentArray}" var="atta">
							...
							<a href="${ctx}/oa/pr/deleteAttachment?attachment4=${fns:urlEncode(atta)}&applicationNo=${pr.applicationNo}" onclick="return confirmx('<spring:message code="确认要删除该附件吗？"></spring:message>', this.href)" ><i class="fa fa-trash"></i> </a>
						<br/>
						</c:forEach>
		...
		<div class="form-actions" style="padding:0;text-align:center;">
				<c:if test="${pr.act.taskDefKey ne 'apply_end'}">
					<input id="btnSubmit" class="btn btn-primary" type="submit" value="<spring:message code='重新提交'></spring:message>" onclick="$('#flag').val('yes')"/>&nbsp;
					<input id="btnSubmit" class="btn btn-inverse" type="submit" value="<spring:message code='撤销'></spring:message>" onclick="$('#flag').val('no')"/>&nbsp;
				</c:if>
