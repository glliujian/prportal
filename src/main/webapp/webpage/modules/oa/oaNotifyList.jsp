...
<div class="ibox-title">
		<h5><spring:message code="通知列表"></spring:message> </h5>
	...
	<div class="col-sm-12">
		...
		<div class="form-group">
			<span><spring:message code="标题"></spring:message>：</span>
				<form:input path="title" htmlEscape="false" maxlength="200"  class=" form-control input-sm"/>
			
			<span><spring:message code="类型"></spring:message>：</span>
				<form:select path="type"  class="form-control m-b">
					<form:option value="" label=""/>
					<form:options items="${fns:getDictList('oa_notify_type')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
				</form:select>
			<c:if test="${!requestScope.oaNotify.self}"><span><spring:message code="状态"></spring:message>：</span>
	...
	<table id="contentTable" class="table table-striped table-bordered  table-hover table-condensed  dataTables-example dataTable no-footer">
		<thead>
			<tr>
				...
				<th><spring:message code="标题"></spring:message></th>
				<th><spring:message code="类型"></spring:message></th>
				<th><spring:message code="状态"></spring:message></th>
				<th><spring:message code="查阅状态"></spring:message></th>
		...
		<c:forEach items="${page.list}" var="oaNotify">
			<tr>
				...
				<td><a  href="#" onclick="openDialogView('<spring:message code='查看通知'></spring:message>', '${ctx}/oa/oaNotify/${requestScope.oaNotify.self?'view':'form'}?id=${oaNotify.id}','800px', '700px')">
				...
				<c:if test="${!requestScope.oaNotify.self}">
					<shiro:hasPermission name="oa:oaNotify:view">
						<a href="#" onclick="openDialogView('<spring:message code="查看通知"></spring:message>', '${ctx}/oa/oaNotify/form?id=${oaNotify.id}','800px', '700px')" class="btn btn-info btn-xs" ><i class="fa fa-search-plus"></i><spring:message code="查看"></spring:message></a>
					</shiro:hasPermission>
					<shiro:hasPermission name="oa:oaNotify:edit">
    					<a href="#" onclick="openDialog('<spring:message code="修改通知"></spring:message>', '${ctx}/oa/oaNotify/form?id=${oaNotify.id}','800px', '700px')" class="btn btn-success btn-xs" ><i class="fa fa-edit"></i><spring:message code="修改"></spring:message></a>
    				</shiro:hasPermission>
    				<shiro:hasPermission name="oa:oaNotify:del">
						<a href="${ctx}/oa/oaNotify/delete?id=${oaNotify.id}" onclick="return confirmx('<spring:message code="确认要删除该通知吗？"></spring:message>', this.href)"   class="btn btn-danger btn-xs"><i class="fa fa-trash"></i><spring:message code="删除"></spring:message></a>
					</shiro:hasPermission>
				</c:if>
				</td>
			</tr>
