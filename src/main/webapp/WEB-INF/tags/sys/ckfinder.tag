...
<!-- <%@ attribute name="AttachmentAdd" type="java.lang.String" required="false" description="<spring:message code='添加'></spring:message>" %>
<%@ attribute name="AttachmentSelect" type="java.lang.String" required="false" description="<spring:message code='选择'></spring:message>" %>
 -->


<ol id="${input}Preview"></ol><c:if test="${!readonly}"><a href="javascript:" onclick="${input}FinderOpen();" class="btn btn-primary">${selectMultiple? "添加":"选择"}</a>&nbsp;<a href="javascript:" onclick="${input}DelAll();" class="btn btn-default"><spring:message code='清除'></spring:message></a></c:if>
<script type="text/javascript">
	function ${input}FinderOpen(){//<c:if test="${type eq 'thumb'}"><c:set var="ctype" value="images"/></c:if><c:if test="${type ne 'thumb'}"><c:set var="ctype" value="${type}"/></c:if>
		...
		windowOpen(url,"<spring:message code='文件管理'></spring:message>",1000,700);
