...
public class SessionManager extends DefaultWebSessionManager {
	...
	public void validateSessions() {
		...
		if(returnObject!=null && returnObject.getReturnCode().equals("0"))
		{
			...
			{
				if(!UserUtils.getPrincipal().getLoginName().equals(returnObject.getUserinfo().getAad()))
				{
					super.destroy();
				}
				
			}else if (StringUtils.isNotBlank(returnObject.getUserinfo().getUsername()))
			{
				if(!UserUtils.getPrincipal().getLoginName().equals(returnObject.getUserinfo().getUsername()))
				{
					super.destroy();
				}
			}
    ...
    public void setTimeout(SessionKey key, long maxIdleTimeInMillis) {
    	try{
    		...
    		super.setTimeout(key, 24*60*60*1000);
    	}catch (InvalidSessionException e) {
    		// 获取不到SESSION不抛出异常
		}
    }
